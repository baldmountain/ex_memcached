    ...
    case Dict.get(data, key) do
      ...
      {value, timestamp, flags, exptime, cas} ->
        case check_expiration(value, timestamp, exptime) do
          nil ->
            { :reply, :not_found, {HashDict.delete(data, key), WorkerState.allocated_change(WorkerState.miss(work_state), -size(value))} }
          value ->
  ...
  def handle_call({:set, key, value, flags, exptime, cas}, _from, {data, work_state}) do
    ...
    case cas do
      0 ->
        ...
        { :reply, {:stored, work_state.current_cas}, {data, WorkerState.allocated_change(WorkerState.next_cas(work_state), size(value))} }
      _ ->
        case Dict.get(data, key) do
          nil ->
          ...
          {old_value, _, _, _, ^cas} ->
            data = Dict.put(data, key, {value, generate_expire_time(exptime), flags, exptime, work_state.current_cas})
            { :reply, {:stored, work_state.current_cas}, {data, WorkerState.allocated_change(WorkerState.next_cas(work_state), size(value)-size(old_value))} }
          {_, _, _, _, _} ->
  ...
  def handle_call({:replace, key, value, flags, expirary}, _from, {data, work_state}) do
    case Dict.get(data, key) do
      ...
      {old_value, timestamp, _, exptime, _} ->
        case check_expiration(old_value, timestamp, exptime) do
          nil ->
            { :reply, :not_stored, {data, WorkerState.allocated_change(work_state, -size(old_value))} }
          _ ->
            data = Dict.put(data, key, {value, generate_expire_time(expirary), flags, expirary, work_state.current_cas})
            { :reply, {:stored, work_state.current_cas}, {data, WorkerState.allocated_change(WorkerState.next_cas(work_state), size(value))} }
        end
  ...
  def handle_call({:cas, key, value, flags, exptime, cas}, _from, {data, work_state}) do
    case Dict.get(data, key) do
      {old_value, _, _, _, ^cas} ->
        ...
        { :reply, {:stored, work_state.current_cas}, {data, WorkerState.allocated_change(work_state, size(value)-size(old_value))} }
      {_, _, _, _, _} ->
        { :reply, :exists, {data, work_state} }
      _ ->
        { :reply, :not_found, {data, work_state} }
  ...
  def handle_call({:delete, key}, _from, {data, work_state}) do
    case Dict.get(data, key) do
      {value, timestamp, _, exptime, _} ->
        case check_expiration(value, timestamp, exptime) do
          nil ->
            { :reply, :not_found, {HashDict.delete(data, key), WorkerState.allocated_change(work_state, -size(value))} }
          value ->
            data = Dict.delete(data, key)
            case is_binary(value) do
              true -> { :reply, :deleted, {data, WorkerState.allocated_change(work_state, size(value))} }
      ...
      {value, timestamp, flags, exptime, _} ->
        case check_expiration(value, timestamp, exptime) do
          nil when expiration == 0xffffffff ->
            case is_binary(value) do
              true -> { :reply, :not_found, {HashDict.delete(data, key), WorkerState.allocated_change(work_state, -size(value))} }
          ...
          |> Enum.reduce data, fn(key, data) ->
            case Dict.get(data, key) do
              {value, timestamp, flags, exptime, cas} ->
                case check_expiration(value, timestamp, exptime) do
                  nil ->
                    {data, WorkerState.allocated_change(work_state, -size(value))}
                  _ ->
